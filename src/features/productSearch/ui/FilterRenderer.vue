  ...
  <div>
    ...
    <div v-else-if="filter.type === 'int' || filter.type === 'float'">
      <div class="row items-center q-col-gutter-sm">
        <div class="col">
          <q-input
            :model-value="selectedMin"
            type="number"
            label="От"
            dense
            outlined
            :placeholder="`${filter.data?.min_value || 0}`"
            @update:model-value="updateMin($event)"
          />
        </div>
        <div class="col">
          <q-input
            :model-value="selectedMax"
            type="number"
            label="До"
            dense
            outlined
            :placeholder="`${filter.data?.max_value || 999999}`"
            @update:model-value="updateMax($event)"
          />
        </div>
      </div>
    </div>
    ...
    <div v-else-if="filter.type === 'bool'">
      <q-toggle
        :model-value="selectedBool"
        :label="selectedBool ? 'Да' : 'Нет'"
        @update:model-value="toggleBool($event)"
      />
    </div>
  </div>
</template>
