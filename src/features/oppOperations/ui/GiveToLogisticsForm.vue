...
export default {
  ...
  methods: {
    async submit() {
      if (!this.formData.order_id || !this.formData.logistics_order_id ||
          !this.formData.product_id || !this.formData.count) {
        this.$q.notify({
          type: 'warning',
      ...
      try {
        const result = await oppOperationsApi.giveToLogistics(this.oppId, this.formData)

        if (result.success) {
          this.$q.notify({
            type: 'positive',
            message: 'Товар успешно передан в логистику'
          })
          this.$emit('success')
          this.show = false
          this.resetForm()
          ...
          this.$q.notify({
            type: 'negative',
            message: result.error || 'Ошибка при передаче товара'
          })
        }
      } catch (error) {
        this.$q.notify({
          type: 'negative',
          message: 'Ошибка при передаче товара в логистику'
        })
      } finally {
  ...
  <q-dialog v-model="show" persistent>
    <q-card style="min-width: 500px">
      <q-card-section class="row items-center q-pb-none">
        <div class="text-h6">Передача товара логистике</div>
        <q-space />
